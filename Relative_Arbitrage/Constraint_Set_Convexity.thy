...
lemma inner_outer_quadform:
  fixes m :: "real^'n^'n"
  shows "m \<bullet> outer_prod u u = u \<bullet> (m *v u)"
proof -
  have "m \<bullet> outer_prod u u = (\<Sum>i\<in>UNIV. \<Sum>j\<in>UNIV. m $ i $ j * (u $ i * u $ j))"
    by (simp add: inner_matrix_eq outer_prod_def)
  also have "\<dots> = (\<Sum>i\<in>UNIV. u $ i * (\<Sum>j\<in>UNIV. m $ i $ j * u $ j))"
    by (simp add: sum_distrib_left ac_simps)
  also have "\<dots> = u \<bullet> (m *v u)"
    by (simp add: inner_vec_def matrix_vector_mult_def)
  finally show ?thesis .
qed

lemma inner_sym_part:
  fixes m s :: "real^'n^'n"
  assumes s: "transpose s = s"
  shows "m \<bullet> s = ((1 / 2) *\<^sub>R (m + transpose m)) \<bullet> s"
proof -
  have sym_entries: "s $ j $ i = s $ i $ j" for i j
    by (metis s transpose_def vec_lambda_beta)
  have "transpose m \<bullet> s = (\<Sum>i\<in>UNIV. \<Sum>j\<in>UNIV. m $ j $ i * s $ i $ j)"
    by (simp add: inner_matrix_eq transpose_def)
  also have "\<dots> = (\<Sum>j\<in>UNIV. \<Sum>i\<in>UNIV. m $ j $ i * s $ i $ j)"
    by (rule sum.swap)
  also have "\<dots> = (\<Sum>j\<in>UNIV. \<Sum>i\<in>UNIV. m $ j $ i * s $ j $ i)"
    by (simp add: sym_entries)
  also have "\<dots> = m \<bullet> s"
    by (simp add: inner_matrix_eq)
...
lemma abel_summation_bound:
  fixes \<mu> q :: "nat \<Rightarrow> real" and n K :: nat
  assumes mono: "\<And>i j. i \<le> j \<Longrightarrow> j < n \<Longrightarrow> \<mu> i \<le> \<mu> j"
    and nonneg: "\<And>i. i < n \<Longrightarrow> 0 \<le> \<mu> i"
    and Tb: "\<And>j. j < n \<Longrightarrow> real (n - K - j) \<le> (\<Sum>i = j..<n. q i)"
  shows "(\<Sum>i < n - K. \<mu> i) \<le> (\<Sum>i < n. \<mu> i * q i)"
proof (cases "n = 0")
  case True
  then show ?thesis
    by simp
next
  case False
  define d where "d = (\<lambda>j. if j = 0 then \<mu> 0 else \<mu> j - \<mu> (j - 1))"
  have d_nonneg: "0 \<le> d j" if "j < n" for j
    using nonneg[of 0] mono[of "j - 1" j] that False
    by (auto simp: d_def)
  have d_sum: "(\<Sum>j \<le> i. d j) = \<mu> i" for i
    by (induction i) (auto simp: d_def)
  have key1: "(\<Sum>j<n. d j * (\<Sum>i = j..<n. q i)) = (\<Sum>i<n. \<mu> i * q i)"
  proof -
    have "(\<Sum>j<n. d j * (\<Sum>i = j..<n. q i))
        = (\<Sum>j<n. \<Sum>i<n. (if j \<le> i then d j * q i else 0))"
    proof (intro sum.cong refl)
      fix j assume j: "j \<in> {..<n}"
      have "d j * (\<Sum>i = j..<n. q i) = (\<Sum>i = j..<n. d j * q i)"
        by (simp add: sum_distrib_left)
      also have "\<dots> = (\<Sum>i<n. if j \<le> i then d j * q i else 0)"
        by (intro sum.mono_neutral_cong_left) auto
      finally show "d j * (\<Sum>i = j..<n. q i)
          = (\<Sum>i<n. if j \<le> i then d j * q i else 0)" .
    qed
    also have "\<dots> = (\<Sum>i<n. \<Sum>j<n. (if j \<le> i then d j * q i else 0))"
      by (rule sum.swap)
    also have "\<dots> = (\<Sum>i<n. \<mu> i * q i)"
    proof (intro sum.cong refl)
      fix i assume i: "i \<in> {..<n}"
      have "(\<Sum>j<n. if j \<le> i then d j * q i else 0) = (\<Sum>j \<le> i. d j * q i)"
        using i by (intro sum.mono_neutral_cong_right) auto
      also have "\<dots> = (\<Sum>j \<le> i. d j) * q i"
        by (simp add: sum_distrib_right)
      also have "\<dots> = \<mu> i * q i"
        by (simp add: d_sum)
      finally show "(\<Sum>j<n. if j \<le> i then d j * q i else 0) = \<mu> i * q i" .
    qed
    finally show ?thesis .
  qed
  have key2: "(\<Sum>j<n. d j * real (n - K - j)) = (\<Sum>i < n - K. \<mu> i)"
  proof -
    have "(\<Sum>j<n. d j * real (n - K - j))
        = (\<Sum>j<n. \<Sum>i < n - K. (if j \<le> i then d j else 0))"
    proof (intro sum.cong refl)
      fix j assume j: "j \<in> {..<n}"
      have "(\<Sum>i < n - K. (if j \<le> i then d j else 0))
          = (\<Sum>i \<in> {..< n - K} \<inter> {i. j \<le> i}. d j)"
        by (simp add: sum.If_cases)
      also have "{..< n - K} \<inter> {i. j \<le> i} = {j..< n - K}"
        by auto
      also have "(\<Sum>i \<in> {j..< n - K}. d j) = real (n - K - j) * d j"
        by simp
      finally show "d j * real (n - K - j)
          = (\<Sum>i < n - K. (if j \<le> i then d j else 0))"
        by (simp add: mult_ac)
    qed
    also have "\<dots> = (\<Sum>i < n - K. \<Sum>j<n. (if j \<le> i then d j else 0))"
      by (rule sum.swap)
    also have "\<dots> = (\<Sum>i < n - K. \<mu> i)"
    proof (intro sum.cong refl)
      fix i assume i: "i \<in> {..< n - K}"
      then have "i < n"
        by auto
      then have "(\<Sum>j<n. if j \<le> i then d j else 0) = (\<Sum>j \<le> i. d j)"
        by (intro sum.mono_neutral_cong_right) auto
      then show "(\<Sum>j<n. if j \<le> i then d j else 0) = \<mu> i"
        by (simp add: d_sum)
    qed
    finally show ?thesis .
  qed
  have "(\<Sum>j<n. d j * real (n - K - j)) \<le> (\<Sum>j<n. d j * (\<Sum>i = j..<n. q i))"
    by (intro sum_mono mult_left_mono Tb d_nonneg) auto
  with key1 key2 show ?thesis
    by simp
qed

subsection \<open>The separation argument\<close>

theorem lemma_2_1_hard:
  fixes k :: nat
  assumes k: "k < CARD('n::finite)"
  shows "(Pi_constraint k :: (real^'n^'n) set)
       \<subseteq> closure (convex hull (suff_volatile k))"
proof
  fix astar :: "real^'n^'n" assume astar: "astar \<in> Pi_constraint k"
  have psd_astar: "psd astar"
    using astar by (simp add: Pi_constraint_def)
  show "astar \<in> closure (convex hull (suff_volatile k))"
  proof (rule ccontr)
    assume not_in: "astar \<notin> closure (convex hull (suff_volatile k))"
    obtain m c where sep: "m \<bullet> astar < c"
    ...
    have Ms_sym: "transpose Ms = Ms"
      by (simp add: Ms_def transpose_sym_part)
    have Ms_inner: "m \<bullet> s = Ms \<bullet> s" if "transpose s = s" for s :: "real^'n^'n"
      unfolding Ms_def by (rule inner_sym_part[OF that])
    obtain B where B: "onormal B" "span B = UNIV"
      and eig: "\<And>u. u \<in> B \<Longrightarrow> Ms *v u = (u \<bullet> (Ms *v u)) *\<^sub>R u"
      using symmetric_eigenbasis[OF Ms_sym] by metis
    ...
    obtain ws0 where ws0: "set ws0 = B" "distinct ws0"
      using finite_distinct_list[OF finB] by blast
    define ws where "ws = sort_key (\<lambda>u. u \<bullet> (Ms *v u)) ws0"
    have ws: "set ws = B" "distinct ws"
      and ws_sorted: "sorted (map (\<lambda>u. u \<bullet> (Ms *v u)) ws)"
      by (simp_all add: ws_def ws0)
    have len_ws: "length ws = n"
      using ws cardB by (metis distinct_card)
    define \<mu> where "\<mu> = (\<lambda>i. ws ! i \<bullet> (Ms *v (ws ! i)))"
    define q where "q = (\<lambda>i. ws ! i \<bullet> (astar *v (ws ! i)))"
    have nth_mem_B: "ws ! i \<in> B" if "i < n" for i
      using ws len_ws that by (metis nth_mem)
    have inj_nth: "inj_on ((!) ws) {..<n}"
      using ws(2) len_ws by (simp add: inj_on_def nth_eq_iff_index_eq)
    have \<mu>_mono: "\<mu> i \<le> \<mu> j" if "i \<le> j" "j < n" for i j
      using ws_sorted that len_ws
      by (auto simp: \<mu>_def sorted_iff_nth_mono)
    text \<open>All eigenvalues of the separating functional are nonnegative:
      otherwise adding a large multiple of the corresponding eigen-projector
      to a generator would drive \<open>m \<bullet> b\<close> below \<open>c\<close>.\<close>
    obtain T0 where T0: "T0 \<subseteq> B" "card T0 = CARD('n) - k" "finite T0"
    proof -
      have "CARD('n) - k \<le> card B"
        using cardB by (simp add: n_def)
      then show thesis
        by (metis obtain_subset_with_card_n that)
    qed
    have onT0: "onormal T0"
      using T0(1,3) B(1)
      by (auto simp: onormal_def elim: pairwise_subset)
    define b0 where "b0 = (\<Sum>u\<in>T0. outer_prod u u)"
    have b0_gen: "b0 \<in> suff_volatile k"
      unfolding b0_def by (rule onormal_sum_suff_volatile[OF onT0 T0(2)])
    have mu_nonneg: "0 \<le> u \<bullet> (Ms *v u)" if u: "u \<in> B" for u
    proof (rule ccontr)
      assume neg: "\<not> 0 \<le> u \<bullet> (Ms *v u)"
      define \<nu> where "\<nu> = u \<bullet> (Ms *v u)"
      have nu_neg: "\<nu> < 0"
        using neg by (simp add: \<nu>_def)
      have outer_sym: "transpose (outer_prod u u) = outer_prod u u"
        by simp
      have m_outer: "m \<bullet> outer_prod u u = \<nu>"
        using Ms_inner[OF outer_sym] inner_outer_quadform[of Ms u]
        by (simp add: \<nu>_def)
      have "c < m \<bullet> (b0 + t *\<^sub>R outer_prod u u)" if t: "0 \<le> t" for t
      ...
      have "m \<bullet> b0 + t0 * \<nu> = c"
        using nu_neg by (simp add: t0_def field_simps)
      with lin[OF t0_nonneg] show False
        by simp
    qed
    text \<open>The lower bound on \<open>\<Sum>\<^sub>i\<^sub>\<ge>\<^sub>j q\<^sub>i\<close> from the \<open>\<Pi>\<close>-constraints, via the
      eigen-projections \<open>P\<^sub>j\<close>.\<close>
    have Uj_facts: "onormal ((!) ws ` {j..<n})" "card ((!) ws ` {j..<n}) = n - j"
      if j: "j \<le> n" for j
    proof -
      have sub: "(!) ws ` {j..<n} \<subseteq> B"
        using nth_mem_B by auto
      show "onormal ((!) ws ` {j..<n})"
        using sub B(1) by (auto simp: onormal_def elim: pairwise_subset)
      have "inj_on ((!) ws) {j..<n}"
        using inj_nth by (auto simp: inj_on_def)
      then show "card ((!) ws ` {j..<n}) = n - j"
        by (simp add: card_image)
    qed
    have T_bound: "real (n - k - j) \<le> (\<Sum>i = j..<n. q i)" if j: "j < n" for j
    proof -
      define U where "U = (!) ws ` {j..<n}"
      have onU: "onormal U" and cardU: "card U = n - j"
        using Uj_facts j by (auto simp: U_def)
      have sum_conv: "(\<Sum>u\<in>U. u \<bullet> (astar *v u)) = (\<Sum>i = j..<n. q i)"
        unfolding U_def q_def
        by (subst sum.reindex) (use inj_nth in \<open>auto simp: inj_on_def\<close>)
      define P where "P = (\<Sum>u\<in>U. outer_prod u u)"
      have projP: "is_proj P" and trP: "trace P = real (n - j)"
        using onormal_proj[OF onU] cardU by (auto simp: P_def)
      have trPa: "trace (astar ** P) = (\<Sum>i = j..<n. q i)"
        by (simp add: P_def trace_mult_outer_sum sum_conv)
      show ?thesis
      proof (cases "j < n - k")
        case True
        have inA: "\<And>m. k < m \<Longrightarrow> m \<le> CARD('n)
            \<Longrightarrow> real (m - k) \<le> Pi_proj astar m"
          using astar by (simp add: Pi_constraint_def)
        have knj: "k < n - j"
          using True by arith
        have njc: "n - j \<le> CARD('n)"
          by (simp add: n_def)
        have "real ((n - j) - k) \<le> Pi_proj astar (n - j)"
          by (rule inA[OF knj njc])
        also have "\<dots> \<le> trace (astar ** P)"
          by (rule Pi_proj_le[OF psd_astar projP trP])
        finally show ?thesis
          by (simp add: trPa diff_right_commute)
      next
        case False
        then have "n - k - j = 0"
          by simp
        moreover have "0 \<le> (\<Sum>i = j..<n. q i)"
          using psd_astar by (intro sum_nonneg) (auto simp: q_def psd_def)
        ultimately show ?thesis
          by simp
      qed
    qed
    text \<open>Assemble: the Frobenius pairing of \<open>Ms\<close> with \<open>astar\<close> dominates the
      pairing with the extremal generator, contradicting separation.\<close>
    have decomp: "Ms = (\<Sum>u\<in>B. (u \<bullet> (Ms *v u)) *\<^sub>R outer_prod u u)"
      by (rule spectral_decomposition[OF B(1) B(2) eig])
    have m_astar: "m \<bullet> astar = (\<Sum>i<n. \<mu> i * q i)"
    proof -
      have outer_astar: "outer_prod u u \<bullet> astar = u \<bullet> (astar *v u)" for u
        by (metis inner_commute inner_outer_quadform)
      have "m \<bullet> astar = Ms \<bullet> astar"
        using psd_astar by (intro Ms_inner) (simp add: psd_def)
      ...
      proof -
        have Bimg: "B = (!) ws ` {..<n}"
          using ws(1) len_ws by (auto simp: set_conv_nth)
        show ?thesis
          unfolding \<mu>_def q_def Bimg
          by (subst sum.reindex[OF inj_nth]) (simp add: o_def)
      qed
      finally show ?thesis .
    qed
    define bstar where "bstar = (\<Sum>u\<in>(!) ws ` {..<n - k}. outer_prod u u)"
    have take_facts: "onormal ((!) ws ` {..<n - k})"
      "card ((!) ws ` {..<n - k}) = CARD('n) - k"
    proof -
      have "{..<n - k} = {0..<n - 0} \<inter> {..<n - k}"
        by auto
      have sub: "(!) ws ` {..<n - k} \<subseteq> B"
        using nth_mem_B by auto
      show "onormal ((!) ws ` {..<n - k})"
        using sub B(1) by (auto simp: onormal_def elim: pairwise_subset)
      have "inj_on ((!) ws) {..<n - k}"
        using inj_nth by (auto simp: inj_on_def)
      then have "card ((!) ws ` {..<n - k}) = n - k"
        by (simp add: card_image)
      then show "card ((!) ws ` {..<n - k}) = CARD('n) - k"
        by (simp add: n_def)
    qed
    have bstar_gen: "bstar \<in> suff_volatile k"
      unfolding bstar_def
      by (rule onormal_sum_suff_volatile[OF take_facts])
    have m_bstar: "m \<bullet> bstar = (\<Sum>i<n - k. \<mu> i)"
    proof -
      have bstar_sym: "transpose bstar = bstar"
        by (simp add: bstar_def transpose_matrix_sum)
      have "m \<bullet> bstar = Ms \<bullet> bstar"
        by (rule Ms_inner[OF bstar_sym])
      also have "\<dots> = (\<Sum>u\<in>(!) ws ` {..<n - k}. u \<bullet> (Ms *v u))"
        by (simp add: bstar_def inner_sum_right inner_commute
            inner_outer_quadform)
      also have "\<dots> = (\<Sum>i<n - k. \<mu> i)"
        unfolding \<mu>_def
        by (subst sum.reindex) (use inj_nth in \<open>auto simp: inj_on_def\<close>)
      finally show ?thesis .
    qed
    have abel: "(\<Sum>i<n - k. \<mu> i) \<le> (\<Sum>i<n. \<mu> i * q i)"
      by (intro abel_summation_bound \<mu>_mono T_bound)
        (auto simp: \<mu>_def intro: mu_nonneg nth_mem_B)
    have "c < m \<bullet> bstar"
      by (rule gens[OF bstar_gen])
    also have "\<dots> = (\<Sum>i<n - k. \<mu> i)"
      by (rule m_bstar)
    also have "\<dots> \<le> (\<Sum>i<n. \<mu> i * q i)"
      by (rule abel)
    also have "\<dots> = m \<bullet> astar"
      by (rule m_astar[symmetric])
    finally show False
      using sep by simp
  qed
qed
...
theorem lemma_2_1:
  fixes k :: nat
  assumes k: "k < CARD('n::finite)"
  shows "convex hull (suff_volatile k) \<subseteq> (Pi_constraint k :: (real^'n^'n) set)"
    and "(Pi_constraint k :: (real^'n^'n) set)
       \<subseteq> closure (convex hull (suff_volatile k))"

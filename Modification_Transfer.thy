...
theorem martingale_of_modification:
  fixes X X' :: "real \<Rightarrow> 'a \<Rightarrow> real"
...
proof -
  ...
  proof (rule SFF.martingale_of_set_integral_eq)
    ...
    proof -
      ...
      qed
      have "(\<integral>\<omega>. indicat_real B \<omega> * (X j \<omega> - X i \<omega>) \<partial>M)
          = (\<integral>\<omega>. indicat_real B \<omega> * X j \<omega>
              - indicat_real B \<omega> * X i \<omega> \<partial>M)"
        by (intro Bochner_Integration.integral_cong refl)
          (simp add: right_diff_distrib)
      also have "\<dots> = (\<integral>\<omega>. indicat_real B \<omega> * X j \<omega> \<partial>M)
          - (\<integral>\<omega>. indicat_real B \<omega> * X i \<omega> \<partial>M)"
        by (intro Bochner_Integration.integral_diff ind_int B_M
            Mg.integrable i j)
      also have "\<dots> = 0"
        using Mg.set_integral_eq[OF B i ij] unfolding conv by simp
      finally show ?thesis .
    qed
    have "(\<integral>\<omega>. indicat_real A \<omega> * (X' j \<omega> - X' i \<omega>) \<partial>M) = 0"
    proof (rule set_integral_zero_transfer
        [where X = X and X' = X' and t = i and D = "\<lambda>\<omega>. X j \<omega> - X i \<omega>"])
      ...
      show "\<And>u. u \<in> {0..i} \<Longrightarrow> AE \<omega> in M. X' u \<omega> = X u \<omega>"
        by (intro ae) auto
      show "integrable M (\<lambda>\<omega>. X j \<omega> - X i \<omega>)"
        by (intro Bochner_Integration.integrable_diff Mg.integrable i j)
      show "integrable M (\<lambda>\<omega>. X' j \<omega> - X' i \<omega>)"
        by (intro Bochner_Integration.integrable_diff int' i j)
      show "AE \<omega> in M. X' j \<omega> - X' i \<omega> = X j \<omega> - X i \<omega>"
        using ae[OF i] ae[OF j] by eventually_elim simp
      show "\<And>B. B \<in> sets (natural_filtration M 0 X i)
          \<Longrightarrow> (\<integral>\<omega>. indicat_real B \<omega> * (X j \<omega> - X i \<omega>) \<partial>M) = 0"
        by (rule zero)
      show "A \<in> sets (natural_filtration M 0 X' i)"
        by (rule A)
    qed
    then have "(\<integral>\<omega>. indicat_real A \<omega> * X' j \<omega> \<partial>M)
        - (\<integral>\<omega>. indicat_real A \<omega> * X' i \<omega> \<partial>M) = 0"
    proof -
      assume z: "(\<integral>\<omega>. indicat_real A \<omega> * (X' j \<omega> - X' i \<omega>) \<partial>M) = 0"
      have "(\<integral>\<omega>. indicat_real A \<omega> * (X' j \<omega> - X' i \<omega>) \<partial>M)
          = (\<integral>\<omega>. indicat_real A \<omega> * X' j \<omega>
              - indicat_real A \<omega> * X' i \<omega> \<partial>M)"
        by (intro Bochner_Integration.integral_cong refl)
          (simp add: right_diff_distrib)
      also have "\<dots> = (\<integral>\<omega>. indicat_real A \<omega> * X' j \<omega> \<partial>M)
          - (\<integral>\<omega>. indicat_real A \<omega> * X' i \<omega> \<partial>M)"
        by (intro Bochner_Integration.integral_diff ind_int A_M int' i j)
      finally show ?thesis using z by simp
    qed
...
section \<open>Vector martingales and their components\<close>

...
| general θ by approximation | **THE ONE OPEN ITEM OF §2.1, and it is missing MATHEMATICS, not missing Isabelle.** Both directions of approximation were checked on 2026-08-08 and both are circular: <br>• from ABOVE (`θ_n = ⌈2ⁿθ⌉/2ⁿ`, finite-valued, `θ_n ↓ θ`): `pexit` moves the right way, but on the survival event one needs `θ_n + v(T−θ_n, X_{θ_n}) ≥ θ + v(T−θ, X_θ)`, which IS the DPP between `θ` and `θ_n`; <br>• from BELOW (floor): same identity with the roles swapped, again the DPP at a random time. <br>No well-founded ordering makes either instance smaller. **One lead worth trying first:** `paper_v k L S K y ≤ paper_v k L (S−h) K y + h` (horizon-Lipschitz) is provable cheaply — `pexit S K ω ≤ pexit (S−h) K (pcut (S−h) ω) + h` plus `paper_pair_class_pcut` — and it kills the HORIZON half of the gap. What it does NOT kill is the SPACE half, `v(T−θ_n, X_θ)` vs `v(T−θ_n, X_{θ_n})`, since the path moves on `(θ, θ_n]`. Closing that needs a modulus for `paper_v` in the space variable along the path, which nobody has. <br>Before attacking it in general, check whether §3.2 needs only `τ_{B_ε}`: there the path is INSIDE the ball up to `τ`, so `|X_θ − X_{θ_n}|` may be controllable by `ε` in a way a general `θ` does not allow |

...
| | clause | status |
|---|---|---|
| (0) | `v < ⊤` | **DONE** for `val_fn` / `stopped_val_fn`; for `paper_v` it is `paper_v ≤ ennreal T` and is not yet stated (one lemma, §2.3) |
...
| (3) | `v = 0` on `K − interior K` | ball case **DONE** for `val_fn`/`stopped_val_fn`; interior value REALIZED for `n−k=1` (`Theorem_1_1.stopped_val_fn_ball_eq_2d`); general `n−k ≥ 2` **OPEN**, §2.2; transfer to `paper_v` §2.3 |

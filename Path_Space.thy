...
text \<open>
  Plan step A5 (STATUS.md 25h), first brick: the space the martingale laws of
  Lemma 2.2 of arXiv:2512.17702 live on. We use HOL-Analysis' set-based
  \<open>cfunspace\<close> (bounded continuous maps with the sup metric) over the compact
  interval \<open>{0..T}\<close>, valued in a Polish type. Everything substantial exists:
  completeness is \<open>Metric_space.mcomplete_cfunspace\<close> (HOL-Analysis) and
  separability is \<open>Metric_space.separable_space_cfunspace\<close>
  (AFP Standard_Borel_Spaces) — target separable and complete, domain compact
  metrizable. These are exactly the two hypotheses of the AFP's
  \<open>Prokhorov_theorem_LP\<close>, so tightness of the Lemma 2.2 laws (from the
  A3+A4 modulus tail bounds) will convert to relative compactness in the
  L\'evy-Prokhorov metric on this space.
\<close>
...
text \<open>
  A5c, first half: the process-to-path map \<open>\<omega> \<mapsto> restrict (\<lambda>t. X t \<omega>) {0..T}\<close> is
  measurable into the Borel \<open>\<sigma>\<close>-algebra of the path space. The route: by
  separability the Borel algebra is generated by the countable family of balls
  around a countable dense set (\<open>generated_by_countable_balls\<close> +
  \<open>borel_of_second_countable'\<close>), and the sup-distance to a FIXED continuous path
  is determined by its values at rational times, so ball preimages are countable
  intersections/unions of one-time-point constraints.
\<close>
...
text \<open>
  ...
  projective-family consistency that step (iii) of the A5d plan (STATUS.md
  25m) feeds to \<open>HOL-Probability.Projective_Limit\<close>.
\<close>
